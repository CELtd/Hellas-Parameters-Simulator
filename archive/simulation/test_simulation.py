...
def test_theoretical_equilibrium():
    ...
    assert eq.v_star == eq.theta, f"At equilibrium, v* should equal theta"

    print(f"  v* = {eq.v_star:.6f}")
    print(f"  q* = {eq.q_star:.6f}")
    print(f"  Δ  = {eq.Delta:.2f}")
...
def main():
    ...
    if failed == 0:
        ...
        print("\nNext steps:")
        print("  1. Run 'streamlit run app.py' for the interactive dashboard")
        print("  2. Open notebooks/hellas_abm_experiments.ipynb for examples")
